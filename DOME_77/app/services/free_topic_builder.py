...
ALLOWED_TYPES = {"passive","voice_answer","choice","drag_drop","visual_pack","memory","drawing","video","roleplay","mini_game"}
INTERACTIVE = {"voice_answer","choice","drag_drop","visual_pack","memory","drawing","roleplay","mini_game"}
...
def _fallback(topic: str, target_language: str, native_language: str, age: int | None, level: str, count: int = 21) -> dict:
    ...
    stages=[
      ...
      ('visual_pack','Pack for the adventure',f'Choose the three useful objects for the next part of the adventure and put them into the adventure bag.'),
    ...
    for i in range(count):
        ...
        if kind=='visual_pack':
            candidates=[w[1],w[3],w[4],w[5]]; slide['items']=candidates; slide['correct_indices']=[0,1,2]; slide['item_image_prompts']=[f'isolated {x}' for x in candidates]; slide['can_skip']=True
...
def _normalize(data: dict, topic: str, target_language: str, native_language: str) -> dict:
    ...
    for i,s in enumerate(slides,1):
        ...
        if s['type']=='visual_pack':
            items=[str(x).strip() for x in (s.get('items') or []) if str(x).strip()]
            if len(items)<3: raise ValueError('visual_pack needs at least 3 objects')
            s['items']=items[:6]
            good=[int(x) for x in (s.get('correct_indices') or []) if isinstance(x,(int,float,str)) and str(x).isdigit()]
            s['correct_indices']=[x for x in good if 0<=x<len(s['items'])][:4] or list(range(min(3,len(s['items']))))
            prompts=[str(x).strip() for x in (s.get('item_image_prompts') or []) if str(x).strip()]
            s['item_image_prompts']=(prompts+['']*len(s['items']))[:len(s['items'])]
...
async def build_free_topic_lesson(topic: str, *, target_language: str, native_language: str, age: int | None, level: str, can_read_target: bool | None = None, slide_count: int = 21) -> dict:
    ...
    instructions=(
        ...
        f"Exactly {slide_count} slides. Learner age={age or 'unknown'}, level={level}, study language={target_language}, support/native language={native_language}, can_read_study_language={bool(can_read_target)}. Topic={topic}. "
        ...
        "Slide 5 MUST be a visual_pack task like the suitcase activity: 4-6 concrete pictured objects, 2-4 correct objects, child drags/taps the useful objects into one destination; provide items, correct_indices and item_image_prompts. Do not require reading to solve it. Use at least 2 concrete choice tasks with real options, 1 other drag_drop with real items/targets, 2 memory tasks with real pairs, 1 drawing, 1 video stage (video_query field describing what clip is needed), 1 roleplay, passive illustrated teaching. If can_read_study_language is false, never make reading essential to completing an activity; use pictures/audio first and text only as optional support. "
        ...
        "Output shape: {title,story_summary,slides:[{id,order,type,title,prompt,support_text,audio_text,image_prompt,video_query,options,correct_option_index,items,targets,correct_indices,item_image_prompts,accepted_meaning,target_phrase,companion_reply,expects_answer,can_skip,required_cartoon_line}]}"
    )
