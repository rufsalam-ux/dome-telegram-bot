...
from app.services.authored_content import persistent_lessons_root
...
async def lesson(request:web.Request)->web.Response:
    await _parent(request); lid=request.match_info['lesson_id']
    pinned_version = request.query.get('version') or request.query.get('revision')
    data = None
    if pinned_version:
        root = persistent_lessons_root() / lid / "_versions"
        if root.exists():
            for vdir in root.iterdir():
                if not vdir.is_dir():
                    continue
                v_json = vdir / "lesson.json"
                if v_json.exists():
                    try:
                        v_data = json.loads(v_json.read_text('utf-8'))
                        if str(v_data.get('revision')) == str(pinned_version) or vdir.name == str(pinned_version):
                            data = v_data
                            break
                    except Exception:
                        pass
...
async def content_manifest(request: web.Request) -> web.Response:
    ...
    The mobile app polls this endpoint (unauthenticated, cheap) to detect
    whether published lesson content has changed since the last sync.  Only
    published lessons stored in persistent_lessons_root() are included.

    Response shape:
        {
          "content_version": "<sha256 of all lesson revisions>",
          "lessons": { "<lesson_id>": { "revision": int, "version": str } }
        }

    Supports ETag / If-None-Match for 304 short-circuit.
    """
    lessons_root = persistent_lessons_root()
    lesson_versions: dict[str, dict] = {}
    for lesson_dir in sorted(lessons_root.iterdir()) if lessons_root.exists() else []:
        lesson_json = lesson_dir / "lesson.json"
        if not lesson_json.exists():
            continue
        try:
            data = json.loads(lesson_json.read_text("utf-8"))
        except Exception:
            continue
        if str(data.get("engine") or "").lower() != "content_v1":
            continue
        status = str(data.get("import_status") or data.get("status") or "").upper()
        if status not in ("PUBLISHED", "ACTIVE"):
            continue
        rev = int(data.get("revision") or 1)
        lesson_versions[lesson_dir.name] = {
            "revision": rev,
            "version": f"{lesson_dir.name}:r{rev}",
        }
    combined = json.dumps(lesson_versions, sort_keys=True, separators=(",", ":"))
    content_version = hashlib.sha256(combined.encode("utf-8")).hexdigest()[:40]
