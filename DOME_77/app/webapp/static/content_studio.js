...
const state={token:sessionStorage.getItem("dome_studio_token")||"",lessons:[],lesson:null,summary:null,lessonId:"",versions:[],media:[],dirty:false,dialogMode:"slide",lessonDialogMode:"create",dragIndex:null,blobUrls:new Map()};
...
async function openLesson(id){
  ...
  try{releaseBlobs();const data=await api(`/api/studio/lessons/${id}`);state.lesson=deepCopy(data.lesson);state.summary=data.summary||{};state.lessonId=id;state.versions=data.versions||[];state.media=data.media||[];state.dirty=false;
    ...
    $("#lessonTargetLanguage").value=state.lesson.target_language||"en";$("#lessonExplanationLanguage").value=state.lesson.explanation_language||state.lesson.native_language||"ru";$("#lessonDifficulty").value=state.lesson.difficulty||"PRE_A1";
    $("#toggleLessonButton").textContent=state.summary.published?"Отключить":"Включить";$("#deleteLessonButton").disabled=state.summary.source!=="persistent";
...
function interactionValue(step){const value=String(step.interaction_type||step.pedagogical_intent||"answer_question");return value==="ask_person_question"?"ask_question":value}
...
function renderStep(step,index){
  ...
  node.querySelector("[data-field=pedagogical_goal]").value=valueFrom(step,"pedagogical_goal",["task_goal","target_meaning"]);node.querySelector("[data-field=interaction_type]").value=interactionValue(step);
  node.querySelector("[data-field=ai_instruction]").value=valueFrom(step,"ai_instruction",["tutor_instruction"]);node.querySelector("[data-field=target_phrase]").value=valueFrom(step,"target_phrase",["bot_says_target"]);node.querySelector("[data-field=native_explanation]").value=valueFrom(step,"native_explanation",["bot_says_native","native_hint"]);node.querySelector("[data-field=hint_example_target]").value=valueFrom(step,"hint_example_target",["hint_target"]);
  ...
  node.querySelector("[data-list=model_examples]").value=listText(step.model_examples||step.target_language_options);node.querySelector("[data-list=options]").value=listText(step.selection_options||step.options);node.querySelector("[data-list=drag_items]").value=listText(step.drag_items||step.items);node.querySelector("[data-list=drag_targets]").value=listText(step.drag_targets||step.targets);
  node.querySelector("[data-config=open_question_first]").checked=step.open_question_first!==false;node.querySelector("[data-config=examples_allowed]").checked=step.examples_allowed!==false;node.querySelector("[data-config=adaptive_scaffolding]").checked=step.adaptive_scaffolding!==false&&step.adaptive!==false;
  ...
  node.querySelector("[data-video=autoplay]").checked=step.autoplay!==false;node.querySelector("[data-video=skippable]").checked=step.skippable!==false;node.querySelector("[data-video=replay]").checked=step.replay!==false;node.querySelector("[data-video=auto_continue]").checked=step.auto_continue!==false&&step.autoContinue!==false;
  ...
  node.querySelectorAll("[data-config]").forEach(field=>field.addEventListener("change",event=>updateConfig(index,event.target.dataset.config,event.target.checked)));
...
function updateField(index,key,value){const step=steps()[index];step[key]=value;if(key==="target_phrase"){step.bot_says_target=value;if(!step.pedagogical_goal)step.task_goal=value;if(!step.prompt)step.prompt=value}if(key==="pedagogical_goal"){step.task_goal=value;step.target_meaning=value}if(key==="interaction_type")step.pedagogical_intent=value==="ask_question"?"ask_person_question":value;if(key==="hint_example_target")step.hint_target=value;if(key==="native_explanation"){step.bot_says_native=value;step.native_hint=value}if(key==="ai_instruction")step.tutor_instruction=value;if(key==="type")renderSteps();else{const card=document.querySelector(`.step-card[data-index="${index}"]`);if(card)card.querySelector(".step-name").textContent=valueFrom(step,"target_phrase",["bot_says_target","task_goal","prompt"])||step.slide_id}markDirty()}
...
function updateConfig(index,key,value){const step=steps()[index];step[key]=Boolean(value);if(key==="adaptive_scaffolding")step.adaptive=Boolean(value);markDirty()}
...
function updateList(index,key,value){const step=steps()[index];const values=lines(value);
  ...
  if(key==="drag_targets"){step.drag_targets=values.map((label,i)=>({id:slug(label,i),label,label_ru:label}));step.targets=deepCopy(step.drag_targets)}
  if(key==="model_examples"){step.model_examples=values;step.target_language_options=values}
  markDirty()
}
function updateVideo(index,key,value){const step=steps()[index];step[key]=Boolean(value);if(key==="auto_continue")step.autoContinue=Boolean(value);const descriptor=Array.isArray(step.media_sequence)?step.media_sequence.find(item=>item?.type==="video"):null;if(descriptor){descriptor[key]=Boolean(value);if(key==="auto_continue")descriptor.autoContinue=Boolean(value)}markDirty()}
...
function openAddDialog(mode){state.dialogMode=mode;$("#stepDialogTitle").textContent=mode==="video"?"Добавить видео":mode==="exercise"?"Добавить задание":"Добавить слайд";$("#confirmAddStep").textContent=mode==="video"?"Добавить и сохранить":"Добавить";$("#slideFields").classList.toggle("hidden",mode==="video");$("#videoFields").classList.toggle("hidden",mode!=="video");fillPosition($("#slidePosition"));fillPosition($("#videoPosition"));$("#newSlideType").innerHTML=taskTypeOptions(mode==="exercise"?"voice_answer":"passive");$("#newSlideFile").value="";$("#newVideoFile").value="";$("#newTargetPhrase").value="";$("#newNativeExplanation").value="";$("#newAiInstruction").value="";$("#stepDialog").showModal()}
async function confirmAdd(event){event.preventDefault();const button=$("#confirmAddStep");button.disabled=true;button.textContent="Добавляем…";
  try{
    ...
    if(state.dialogMode==="video"){
      const file=$("#newVideoFile").files?.[0];if(!file)throw new Error("Выберите видеофайл MP4, MOV или WebM.");const asset=await uploadFile(file);position=Number($("#videoPosition").value);const autoContinue=$("#videoAutoContinue").checked;step={slide_id:uniqueId("video"),type:"video",src:asset.path,video_file:asset.path,autoplay:$("#videoAutoplay").checked,auto_continue:autoContinue,autoContinue,skippable:$("#videoSkippable").checked,replay:$("#videoReplay").checked,aspect_ratio:"16:9",requiredForMovie:false,media_sequence:[{id:"video",type:"video",src:asset.path,autoplay:$("#videoAutoplay").checked,auto_continue:autoContinue,autoContinue,skippable:$("#videoSkippable").checked,replay:$("#videoReplay").checked,aspect_ratio:"16:9"}]};
    }else{
      position=Number($("#slidePosition").value);const target=$("#newTargetPhrase").value.trim(),nativeText=$("#newNativeExplanation").value.trim(),ai=$("#newAiInstruction").value.trim(),type=$("#newSlideType").value;step={slide_id:uniqueId("step"),type,prompt:target||nativeText||ai||"Новый слайд",target_phrase:target,bot_says_target:target,task_goal:target,native_explanation:nativeText,bot_says_native:nativeText,native_hint:nativeText,ai_instruction:ai,tutor_instruction:ai,interaction_type:"answer_question",pedagogical_intent:"answer_question",open_question_first:true,examples_allowed:true,adaptive_scaffolding:true,adaptive:true,requiredForMovie:false,max_attempts:3,controls:{answer:{enabled:type==="voice_answer",required:type==="voice_answer"},continue:{enabled:true,when:type==="voice_answer"?"after_answer":"always"},hint:{enabled:true},follow_up:{enabled:type==="dialogue"}},answer_mode:type==="voice_answer"?"required_voice":"none",continue_policy:type==="voice_answer"?"after_answer":"always",hint_enabled:true,follow_up_policy:type==="dialogue"?"optional":"none",allow_ai_followup:type==="dialogue"};
...
function candidate(){const lesson=deepCopy(state.lesson);lesson.title=$("#lessonTitle").value.trim();lesson.target_language=$("#lessonTargetLanguage").value.trim()||"en";lesson.explanation_language=$("#lessonExplanationLanguage").value.trim()||"ru";lesson.difficulty=$("#lessonDifficulty").value;const key=Array.isArray(lesson.steps)?"steps":"slides";lesson[key]=steps().map((step,index)=>({...deepCopy(step),order:index+1}));return lesson}
...
function openLessonDialog(mode){state.lessonDialogMode=mode;const duplicate=mode==="duplicate";$("#lessonDialogTitle").textContent=duplicate?"Дублировать урок":"Новый урок";$("#confirmLessonAction").textContent=duplicate?"Создать копию":"Создать урок";$("#newLessonId").value=duplicate?`${state.lessonId}_copy`:"";$("#newLessonTitle").value=duplicate?`${state.lesson?.title||state.lessonId} — копия`:"";$("#newLessonTargetLanguage").value=state.lesson?.target_language||"en";$("#newLessonExplanationLanguage").value=state.lesson?.explanation_language||state.lesson?.native_language||"ru";$("#lessonDialog").showModal()}
async function confirmLessonAction(event){event.preventDefault();const lessonId=$("#newLessonId").value.trim().toLowerCase(),title=$("#newLessonTitle").value.trim();if(!/^[a-z0-9][a-z0-9_-]{1,79}$/.test(lessonId)){showErrors("ID урока: минимум 2 символа, только латинские буквы, цифры, _ и -.");return}if(!title){showErrors("Введите название урока.");return}const button=$("#confirmLessonAction");button.disabled=true;
  try{const body={lesson_id:lessonId,title,target_language:$("#newLessonTargetLanguage").value.trim()||"en",explanation_language:$("#newLessonExplanationLanguage").value.trim()||"ru"};const path=state.lessonDialogMode==="duplicate"?`/api/studio/lessons/${state.lessonId}/duplicate`:"/api/studio/lessons";await api(path,{method:"POST",body:JSON.stringify(body)});$("#lessonDialog").close();state.dirty=false;await loadLessons();await openLesson(lessonId);notice(state.lessonDialogMode==="duplicate"?"Копия урока создана как черновик.":"Урок создан. Добавьте первый слайд или видео.")}
  catch(error){showErrors(error.details?.length?error.details:error.message)}finally{button.disabled=false}}
async function toggleLesson(){if(!state.lessonId)return;if(state.dirty&&!await save())return;if(state.summary?.published){if(!confirm("Отключить урок? Новые прохождения не смогут его открыть, данные и версии сохранятся."))return;try{await api(`/api/studio/lessons/${state.lessonId}/archive`,{method:"POST"});await openLesson(state.lessonId);await loadLessons();notice("Урок отключён. Его версии и медиа сохранены.")}catch(error){showErrors(error.details?.length?error.details:error.message)}}else await publish()}
async function deleteLesson(){if(!state.lessonId||state.summary?.source!=="persistent")return;if(!confirm(`Удалить урок «${state.lesson?.title||state.lessonId}» и его черновики/медиа? Для опубликованного урока безопаснее использовать «Отключить».`))return;if(!confirm("Подтвердите удаление ещё раз. Это действие нельзя отменить через редактор."))return;try{await api(`/api/studio/lessons/${state.lessonId}`,{method:"DELETE"});releaseBlobs();state.lesson=null;state.summary=null;state.lessonId="";state.dirty=false;$("#editorBody").classList.add("hidden");$("#emptyState").classList.remove("hidden");await loadLessons()}catch(error){showErrors(error.details?.length?error.details:error.message)}}

$("#loginButton").addEventListener("click",login);$("#tokenInput").addEventListener("keydown",event=>{if(event.key==="Enter")login()});$("#refreshLessons").addEventListener("click",loadLessons);$("#lessonSearch").addEventListener("input",renderLessonList);$("#newLessonButton").addEventListener("click",()=>openLessonDialog("create"));$("#confirmLessonAction").addEventListener("click",confirmLessonAction);
$("#lessonTitle").addEventListener("input",markDirty);for(const id of ["lessonTargetLanguage","lessonExplanationLanguage","lessonDifficulty"])$("#"+id).addEventListener("input",markDirty);$("#addSlideButton").addEventListener("click",()=>openAddDialog("slide"));$("#addExerciseButton").addEventListener("click",()=>openAddDialog("exercise"));$("#addVideoButton").addEventListener("click",()=>openAddDialog("video"));$("#confirmAddStep").addEventListener("click",confirmAdd);
$("#saveButton").addEventListener("click",save);$("#publishButton").addEventListener("click",publish);$("#previewButton").addEventListener("click",preview);$("#restoreButton").addEventListener("click",showVersions);$("#duplicateLessonButton").addEventListener("click",()=>openLessonDialog("duplicate"));$("#toggleLessonButton").addEventListener("click",toggleLesson);$("#deleteLessonButton").addEventListener("click",deleteLesson);
