...
export function DomePressable({children,style,contentStyle,pressedStyle,feedback=false,onPressIn,onPressOut,disabled,...props}:Props){
  ...
  const pressIn=(event:any)=>{if(disabled)return;setPressed(true);Animated.timing(motion,{toValue:1,duration:72,useNativeDriver:true}).start();if(feedback)emitDomeFeedback(feedback);onPressIn?.(event)};
  const pressOut=(event:any)=>{if(disabled)return;setPressed(false);Animated.spring(motion,{toValue:0,speed:28,bounciness:6,useNativeDriver:true}).start();onPressOut?.(event)};
