...
import {Animated,PanResponder,Pressable,Text,View,Vibration} from 'react-native';
...
function Chip({value,active=false,done=false,onPress,testID}:{value:TaskOption;active?:boolean;done?:boolean;onPress?:()=>void;testID:string}){
  return <Pressable testID={testID} accessibilityRole='button' disabled={done} onPress={onPress} style={{minHeight:50,minWidth:92,flex:1,borderRadius:14,borderWidth:3,borderColor:done?'#13a864':active?'#246bfd':'#d3dae6',backgroundColor:done?'#dff8e9':active?'#eef4ff':'#fff',alignItems:'center',justifyContent:'center',padding:8}}><Text style={{fontSize:16,fontWeight:'800',textAlign:'center',color:'#20243a'}}>{done?'✓ ':''}{value.label}</Text></Pressable>;
}
...
function MemoryTask({slide,initial,onResult,disabled}:{slide:any;initial:any;onResult:(value:TemplateTaskResult)=>void;disabled:boolean}){
  ...
  return <View testID='template-memory' style={{gap:8}}><Text style={{fontWeight:'800'}}>Найди одинаковые пары</Text><View style={{flexDirection:'row',flexWrap:'wrap',gap:8}}>{deck.map(card=>{const visible=open.includes(card.id)||matched.includes(card.pairId);return <Pressable key={card.id} testID={`memory-${card.id}`} accessibilityRole='button' disabled={disabled||matched.includes(card.pairId)} onPress={()=>reveal(card.id,card.pairId)} style={{width:'22%',minWidth:64,aspectRatio:1,borderRadius:14,backgroundColor:visible?'#fff':'#246bfd',borderWidth:3,borderColor:matched.includes(card.pairId)?'#13a864':'#d3dae6',alignItems:'center',justifyContent:'center',padding:5}}><Text style={{fontWeight:'800',textAlign:'center',color:visible?'#20243a':'#fff'}}>{visible?card.value.label:'?'}</Text></Pressable>})}</View></View>;
}
...
function PuzzleTask({slide,initial,onResult,disabled}:{slide:any;initial:any;onResult:(value:TemplateTaskResult)=>void;disabled:boolean}){
  ...
  return <View testID='template-puzzle' style={{gap:8}}><Text style={{fontWeight:'800'}}>Собери части по порядку</Text><View style={{flexDirection:'row',flexWrap:'wrap',gap:5}}>{order.map((piece,index)=><Pressable key={`${piece}-${index}`} testID={`puzzle-position-${index}`} accessibilityRole='button' disabled={disabled} onPress={()=>choose(index)} style={{width:`${Math.floor(96/columns)}%`,aspectRatio:1,borderRadius:10,borderWidth:3,borderColor:selected===index?'#246bfd':'#d3dae6',backgroundColor:'#eef4ff',alignItems:'center',justifyContent:'center'}}><Text style={{fontSize:22,fontWeight:'900'}}>{piece+1}</Text></Pressable>)}</View></View>;
}
