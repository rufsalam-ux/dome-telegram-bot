...
export type StartupStage='ENTRY_EVALUATION'|'APP_MODULE_LOADED'|'APP_MODULE_LOAD_FAILED'|'ROOT_REGISTERED'|'APP_MOUNT'|'APP_RUNTIME_LOADED'|'APP_RUNTIME_LOAD_FAILED'|'ROOT_RUNTIME_FAILED'|'ROOT_RUNTIME_RETRY'|'SECURESTORE_DONE'|'BACKEND_BOOTSTRAP_DONE'|'NAV_READY'|'FIRST_SCREEN_RENDERED';

export type StartupFailure={code:string;reason:string;message:string};
...
function shortReason(value:string):string{
  return value.replace(/\s+/g,' ').trim().slice(0,120)||'Unknown startup error';
}

export function rootRuntimeFailure(error:unknown):StartupFailure{
  ...
  const code=/expoaudio|expo-audio|audioplayer|shared.?object|native module.+audio/i.test(searchable)
    ...
    :/loading chunk|dynamic import|module.+(load|resolve)|unable to resolve/i.test(searchable)
      ?'APP_RUNTIME_LOAD'
      :'ROOT_RUNTIME';
  return {code,reason,message:'Не удалось открыть приложение. Попробуйте ещё раз.'};
}
  ...
  const code=error instanceof StartupTimeoutError
    ...
    :/network request failed|failed to fetch|networkerror/i.test(reason)
      ...
      :/secure.?store|keystore|keychain/i.test(reason)
        ?'SECURESTORE_ERROR'
        :'BOOTSTRAP_ERROR';
  return {code,reason,message:startupErrorText(error)};
}
