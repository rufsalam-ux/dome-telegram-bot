...
export function AppStoreProvider({children}:{children:React.ReactNode}){
 ...
 const hydrate=async(data:any,t:string)=>{setApiToken(t);setToken(t);await SecureStore.setItemAsync('dome_mobile_token',t);setParent(data.parent);const rawChildren=Array.isArray(data.children)?data.children:[];const cs=rawChildren.filter((x:any)=>x&&x.id!=null&&String(x.name||x.display_name||'').trim()).map((x:any)=>({id:String(x.id),parentId:String(data.parent?.id||''),name:String(x.name||x.display_name).trim(),age:x.age??x.age_years,learningLanguage:x.target_language||'ru',nativeLanguage:x.native_language||'ru',courseId:'conversation',activeCharacterId:x.active_character_id,heroUrl:x.hero_url||null}));setChildren(cs);setSelectedChild(undefined);setScreen('children')};
