...
def build_free_topic_cartoon(
    ...
    output:Path, duration:int=75, companion_png:Path|None=None, first_child_scene_seconds:int=8,
)->Path:
    ...
    first_child_scene_seconds=max(8,int(first_child_scene_seconds or 8))
    ...
    # Body bob + horizontal walk. For a single flat PNG true skeletal arm/leg animation is
    # impossible without segmented assets, so we don't pretend otherwise.
    filt=[
        "[1:v]format=rgba,scale=-1:330,rotate='0.010*sin(3*t)':ow=rotw(iw):oh=roth(ih):c=none[hero]",
        f"[0:v][hero]overlay=x='80+90*sin(0.18*t)':y='360+8*abs(sin(4*t))':enable='between(t,0,{duration})'[v0]",
    ]
    vlabel='v0'
