...
async def _evaluate_with_chat(prompt: dict) -> dict | None:
    instructions = (
        ...
        "Sometimes ask a natural follow-up, sometimes simply acknowledge and continue. "
        "When the child needs help, give an actual usable example or choice instead of merely promising help. "
        "Use the child's name only occasionally when a name is provided, never in every reply. "
        "At low difficulty accept one-word/very short answers; at high difficulty invite a reason, detail, comparison or short dialogue. "
        "response_native may briefly explain the result in the child's native language."
    )
